...
class CascadeRegression(InMemoryDataset):
    ...
    def processed_dir(self):
        return os.path.join(self.root, "processed", "inMemory", self.task, self.name, str(self.observation))
    ...
    def __repr__(self):
        return f'{self.__class__.__name__} \nnetwork: {self.name} \ntask: {self.task} \nobservation-window t={str(self.observation)} \n(Number of graphs: {len(self)}, Number of features [activation, deg cent, eigen cent, btwns cemt]: {self.w})'
...
class CascadeDataset(Dataset):
    def __init__(self, root, name, edge_index_path, task, observation, directed):
        ...
        self.directed = directed
        self.edge_index_tensor = self.get_edge_index(edge_index_path)
        super(CascadeDataset, self).__init__(root)
    ...
    @property
    def processed_file_names(self):
        cascade_file_path = self.raw_paths[0]
        with open(cascade_file_path, 'r') as f:
            cascade_data = f.readline()
        return [f'data_{i}.pt' for i in range (len(cascade_data))]
    ...
    def process(self):
        ...
        for index, cascade in enumerate(cascades_data):
            ...
            data = Data(x=X, y=y, edge_index=self.edge_index_tensor, cascade_name=data_name)
            torch.save(data, os.path.join(self.processed_dir, f'data_{index}.pt'))
        
    def len(self):
        cascades_file_path = self.raw_paths[0]
        with open(cascades_file_path, 'r') as file:
            cascades_data = file.readlines()
        return len(cascades_data)
